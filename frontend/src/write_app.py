...
// ═══════════════════════════════════════════════════════════════════════════════
// EXPLAINER BUTTON — context-sensitive help for every tab
// ═══════════════════════════════════════════════════════════════════════════════
const EXPLAINER_CONTENT={
  dashboard:{title:"Dashboard",what:"Shows the week's overall schedule performance — efficiency score, capacity vs demand, and any gaps that need attention.",steps:["Check the efficiency score (aim for >80%)","Review the capacity vs demand bars for each day","Look for red unassigned slots — these need OT or reoptimisation","Use the Generate Schedule button to rebuild if staff or demand has changed"]},
  demand:{title:"Demand — Weekly Planner",what:"Converts your Yearly Demand Planner (YDP) weekly hour totals into daily allocations, distributed across days based on your Crop Activity Standards.",steps:["Go to Labour Planner → Yearly Demand Planner → populate your plans first","Set Crop Activity Standards (which days each activity runs)","Review the weekly grid — hours appear automatically from the YDP","Confirm each week you are ready to schedule","Generate Schedule uses confirmed weeks only"]},
  schedule:{title:"Schedule",what:"The optimised weekly staff schedule generated by the CP-SAT solver. Shows who works where, on which activity, on each day.",steps:["Review GH / Staff / Activity views using the sub-tabs","Red unassigned slots mean demand could not be covered by regular hours","Go to Overtime tab to cover unassigned demand","Use Reoptimise for a specific staff member after marking an absence"]},
  overtime:{title:"Overtime",what:"Shows labour that could not be covered by regular staff hours. Managers can manually assign OT or let the system suggest fair allocation. Includes a ballpark cost estimate.",steps:["Click 'Calculate OT' to surface unmet demand","Review OT Balance to see who has capacity","Manually add entries or accept system suggestions","Set the average OT rate for a rough weekly cost estimate","Lock entries once approved to prevent accidental changes"]},
  edit:{title:"Edit — Staff & Setup",what:"Configure all staff profiles, greenhouses, activities, and crop types. Changes here affect scheduling and all demand calculations.",steps:["Add or edit staff — set daily hours, OT limit, OT willing flag","Tick Sat Regular / Sun Regular / Holiday Worker for staff on permanent weekend or holiday rosters","Set which crops and activities each person can do","Configure greenhouses with crop type assignments","Save before generating a new schedule"]},
  quarantine:{title:"Quarantine",what:"Create and manage quarantine orders for greenhouses or individual staff. Three types: A (full GH lockdown), B (individual staff restriction), C (limited activities only in a GH).",steps:["Pick the quarantine type based on the situation","Set the START DATE — it does not have to be today","Set the duration in days — end date is shown automatically","Type A / C: select the greenhouse   Type B: select affected staff","Click Create — the order is tracked in Active Orders below"]},
  backup:{title:"Backup",what:"Export a full data backup to your computer, or import a previously saved backup. Always export before major changes.",steps:["Click Export Backup to download a JSON file","Save the file somewhere safe (email it to yourself, save to Drive etc.)","To restore: click Import Backup and select the saved file","Backups include all staff, schedules, demand, standards and settings","The Vercel link always shows live cloud data — backup is a safety net"]},
  absence:{title:"Absence",what:"Record planned and unplanned staff absences for the week. Absences are factored into the CP-SAT optimiser so work is redistributed automatically.",steps:["Select the day the staff member is absent","Tick their name for a full-day absence or enter partial hours","Add a note if needed","Click Reoptimise on the Schedule tab after recording absences"]},
};
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setOpen(false)}>
          <div style={{background:"white",borderRadius:"14px",padding:"28px",maxWidth:"500px",width:"92%",maxHeight:"82vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"18px"}}>
              <h3 style={{color:navy,margin:0,fontSize:"16px"}}>📖 {c.title}</h3>
              <button onClick={()=>setOpen(false)} style={{background:"none",border:"none",fontSize:"20px",cursor:"pointer",color:"#999",lineHeight:1}}>✕</button>
            </div>
            <div style={{marginBottom:"18px"}}>
              <div style={{fontSize:"11px",fontWeight:"700",color:navy,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:"8px"}}>What this tab does</div>
              <p style={{color:"#374151",fontSize:"13px",lineHeight:"1.65",margin:0}}>{c.what}</p>
            </div>
            <div>
              <div style={{fontSize:"11px",fontWeight:"700",color:navy,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:"8px"}}>How to use it</div>
              <ol style={{color:"#374151",fontSize:"13px",lineHeight:"1.9",margin:0,paddingLeft:"20px"}}>
                {c.steps.map((s,i)=><li key={i} style={{marginBottom:"3px"}}>{s}</li>)}
              </ol>
            </div>
          </div>
        </div>
    ...
    <div style={{position:"relative"}}>
      ...
      <div style={card}>
        <div style={{display:"flex",gap:"12px",flexWrap:"wrap",alignItems:"flex-end",marginBottom:"16px"}}>
          <div>
            <label style={{display:"block",fontSize:"12px",color:C.textMid,marginBottom:"4px"}}>Avg OT rate $/hr <span style={{color:C.textLight,fontWeight:"400"}}>(ballpark — applied to all OT hours)</span>:</label>
            <input type="number" min="0" value={defaultRate} onChange={e=>setDefaultRate(e.target.value)} style={{...inp,width:"130px"}} placeholder="e.g. 35"/>
          </div>
            ...
            <tbody>
              {overtimeEntries.map((e,i)=>(
                <tr key={e.id} style={{background:e.locked?"#f0fdf4":e.source==="manual"?"#fdf4ff":i%2===0?C.light:C.white}}>
                  ...
                  <td style={{padding:"8px"}}>
                    <div>{e.day}</div>
                    {e.day==="Saturday"&&<span style={{fontSize:"10px",background:"#fef3c7",color:"#92400e",padding:"1px 6px",borderRadius:"8px",fontWeight:"700"}}>SAT OT</span>}
                    {e.day==="Sunday"&&<span style={{fontSize:"10px",background:"#ffe4e6",color:"#991b1b",padding:"1px 6px",borderRadius:"8px",fontWeight:"700"}}>SUN OT</span>}
                    {!["Saturday","Sunday"].includes(e.day)&&<span style={{fontSize:"10px",background:"#e0f2fe",color:"#0369a1",padding:"1px 6px",borderRadius:"8px",fontWeight:"700"}}>AFTER-HRS</span>}
                  </td>
...
function QuarantinePanel({staff,ghList,quarantine,setQuarantine,quarantineHistory,setQuarantineHistory,schedule,dailyAllocation,setDailyAllocation,confirmedWeeks,setConfirmedWeeks,wsHolidays,activities,addAuditEntry,btn,inp,C,API,adelaideTime,fmtISOReadable,role}){
  ...
  const [typeA_startDate,setTypeA_startDate]=useState(todayISO);
  ...
  const [typeB_startDate,setTypeB_startDate]=useState(todayISO);
  ...
  const [typeC_startDate,setTypeC_startDate]=useState(todayISO);
  const fmtEnd=(startISO,days)=>{try{const d=new Date(startISO);d.setDate(d.getDate()+days);return d.toLocaleDateString("en-AU",{day:"2-digit",month:"short",year:"numeric"});}catch{return "";}};
  ...
  const createTypeA=async()=>{
    ...
    try{
      const res=await axios.post(`${API}/quarantine`,{
        ...
        reason:typeA_reason,startDate:new Date(typeA_startDate).toISOString(),
  ...
  const createTypeB=async()=>{
    ...
    try{
      const res=await axios.post(`${API}/quarantine`,{
        ...
        reason:typeB_reason,startDate:new Date(typeB_startDate).toISOString(),
  ...
  const createTypeC=async()=>{
    ...
    try{
      const res=await axios.post(`${API}/quarantine`,{
        ...
        reason:typeC_reason,startDate:new Date(typeC_startDate).toISOString(),
        quarantineType:"C",
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"20px",marginBottom:"24px"}}>
        ...
        <div style={{border:`2px solid ${C.red}`,borderRadius:"10px",padding:"16px",background:"#fff8f8"}}>
          ...
          <div style={{display:"flex",gap:"10px",marginBottom:"10px",flexWrap:"wrap"}}>
            <div><label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Start Date:</label><input type="date" value={typeA_startDate} onChange={e=>setTypeA_startDate(e.target.value)} style={{...inp,width:"140px"}}/></div>
            <div><label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Duration (days):</label><input type="number" min="1" max="90" value={typeA_days} onChange={e=>setTypeA_days(parseInt(e.target.value)||7)} style={{...inp,width:"70px"}}/></div>
          </div>
          <p style={{color:C.textLight,fontSize:"11px",margin:"-4px 0 10px 0"}}>Ends: <strong>{fmtEnd(typeA_startDate,typeA_days)}</strong></p>
        ...
        <div style={{border:`2px solid ${C.purple}`,borderRadius:"10px",padding:"16px",background:"#fdf8ff"}}>
          ...
          <div style={{display:"flex",gap:"10px",marginBottom:"10px",flexWrap:"wrap"}}>
            <div><label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Start Date:</label><input type="date" value={typeB_startDate} onChange={e=>setTypeB_startDate(e.target.value)} style={{...inp,width:"140px"}}/></div>
            <div><label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Duration (days):</label><input type="number" min="1" max="90" value={typeB_days} onChange={e=>setTypeB_days(parseInt(e.target.value)||7)} style={{...inp,width:"70px"}}/></div>
          </div>
          <p style={{color:C.textLight,fontSize:"11px",margin:"-4px 0 10px 0"}}>Ends: <strong>{fmtEnd(typeB_startDate,typeB_days)}</strong></p>
          <div style={{marginBottom:"12px"}}>
            <label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Reason (optional):</label>
        ...
        <div style={{border:`2px solid ${C.orange}`,borderRadius:"10px",padding:"16px",background:"#fffbf5"}}>
          ...
          <div style={{display:"flex",gap:"10px",marginBottom:"10px",flexWrap:"wrap"}}>
            <div><label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Start Date:</label><input type="date" value={typeC_startDate} onChange={e=>setTypeC_startDate(e.target.value)} style={{...inp,width:"140px"}}/></div>
            <div><label style={{display:"block",fontSize:"12px",fontWeight:"600",color:C.navy,marginBottom:"4px"}}>Duration (days):</label><input type="number" min="1" max="90" value={typeC_days} onChange={e=>setTypeC_days(parseInt(e.target.value)||7)} style={{...inp,width:"70px"}}/></div>
          </div>
          <p style={{color:C.textLight,fontSize:"11px",margin:"-4px 0 10px 0"}}>Ends: <strong>{fmtEnd(typeC_startDate,typeC_days)}</strong></p>
          <div style={{marginBottom:"10px"}}>
      ...
      <div style={{background:"white",borderRadius:"14px",padding:"28px",maxWidth:"820px",width:"95%",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
        ...
        <div style={{marginBottom:"18px",padding:"14px",background:"#fafafa",borderRadius:"8px",border:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",gap:"16px",flexWrap:"wrap",marginBottom:"14px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
              <label style={{display:"flex",alignItems:"center",gap:"6px",cursor:"pointer",fontSize:"13px",color:C.textMid}} title="Saturday is part of this person's regular contract — not OT">
                <input type="checkbox" checked={selectedStaff.satRegular===true} onChange={e=>update({satRegular:e.target.checked})} style={{accentColor:C.orange,width:16,height:16}}/>
                Sat Regular
              </label>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
              <label style={{display:"flex",alignItems:"center",gap:"6px",cursor:"pointer",fontSize:"13px",color:C.textMid}} title="Sunday is part of this person's regular contract — not OT">
                <input type="checkbox" checked={selectedStaff.sunRegular===true} onChange={e=>update({sunRegular:e.target.checked})} style={{accentColor:C.orange,width:16,height:16}}/>
                Sun Regular
              </label>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
              <label style={{display:"flex",alignItems:"center",gap:"6px",cursor:"pointer",fontSize:"13px",color:C.green,width:16,height:16}} title="This person works public holidays as part of their regular contract">
                <input type="checkbox" checked={selectedStaff.holidayWorker===true} onChange={e=>update({holidayWorker:e.target.checked})} style={{accentColor:C.green,width:16,height:16}}/>
                Holiday Worker
              </label>
            </div>
          </div>
